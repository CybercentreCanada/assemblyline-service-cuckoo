...
from assemblyline_v4_service.common.dynamic_service_helper import SandboxOntology, NetworkEvent, ProcessEvent, EVENT
...
def generate_al_result(api_report: Dict[str, Any], al_result: ResultSection, file_ext: str,
    ...
    pgm = PidGuidMap()
    if sysmon:
        add_processes_to_pgm(sysmon, pgm)
        convert_sysmon_processes(sysmon, events, safelist)
        convert_sysmon_network(sysmon, network, events, safelist)
...
def process_behaviour(behaviour: Dict[str, Any], events: List[Dict[str, Any]],
                      safelist: Dict[str, Dict[str, List[str]]], pgm: PidGuidMap) -> None:
    """
...
def convert_cuckoo_processes(events: List[Dict],
    ...
    :return: None
    """
    existing_pids = [proc["pid"] for proc in events if proc.keys() == ProcessEvent.keys()]
    for item in cuckoo_processes:
        ...
        if item["pid"] not in existing_pids:
            ...
            ontology_process = {
                ...
                "guid": pgm.get_guid_by_pid_and_time(
                    item["pid"], item["first_seen"]),
...
def build_process_tree(events: Optional[List[Dict[str, Any]]] = None,
    ...
    if len(events) > 0:
        ...
        process_tree_result_section = so.get_event_tree_result_section(
            signatures=signatures,
            tree_type=EVENT,
            safelist=SAFE_PROCESS_TREE_LEAF_HASHES.keys()
        )
        if is_process_martian:
            sig_name = "process_martian"
            heur_id = get_category_id(sig_name)
            process_tree_result_section.set_heuristic(heur_id)
            # Let's keep this heuristic as informational
            process_tree_result_section.heuristic.add_signature_id(sig_name, score=10)
        parent_result_section.add_subsection(process_tree_result_section)
...
def convert_sysmon_network(sysmon: List[Dict[str, Any]], network: Dict[str, Any], events: List[Dict[str, Any]],
                           safelist: Dict[str, Dict[str, List[str]]]) -> None:
    """
            ...
            if any(network_conn[key] is None for key in network_conn.keys()) or not protocol:
                ...
                for req in network[protocol][:]:
                    if req["dst"] == network_conn["dst"] and \
                        ...
                        events.append({
                            "dest_ip": network_conn["dst"],
                            "image": network_conn["image"],
                            "src_ip": network_conn["src"],
                            "src_port": network_conn["sport"],
                            "guid": network_conn["guid"],
                            "pguid": network_conn["pguid"],
                            "domain": None,
                            "pid": network_conn["pid"],
                            "timestamp": network_conn["time"],
                            "dest_port": network_conn["dport"],
                            "protocol": protocol,
                        })
            else:
                network[protocol].append(network_conn)
            ...
            if any(dns_query[key] is None for key in dns_query.keys()):
                ...
                for query in network["dns"][:]:
                    if query["request"] == dns_query["request"]:
                        ...
                        events.append({
                            "dest_ip": dns_query["answers"][0]["data"] if dns_query.get("answers") else None,
                            "image": dns_query["image"],
                            "src_ip": None,
                            "src_port": None,
                            "guid": dns_query["guid"],
                            "pguid": dns_query["pguid"],
                            "domain": dns_query["request"],
                            "pid": dns_query["pid"],
                            "timestamp": dns_query["time"],
                            "dest_port": None,
                            "protocol": "dns",
                        })
            else:
...
def add_processes_to_pgm(sysmon: List[Dict[str, Any]], pgm: PidGuidMap):
    """
    This method creates the PID -> GUID lookup table
    :param sysmon: A list of processes observed during the analysis of the task by the Sysmon tool
    :param pgm: The Pid -> GUID lookup table
    :return: None
    """
    for event in sysmon:
        ...
        if process["guid"] and f"{{{str(UUID(process['guid']))}}}" in pgm.guid_pid_map.keys():
            ...
            if process_with_guid.start_time == float("-inf") and process["start_time"] != float("-inf"):
                ...
                process_with_guid.end_time = process["end_time"]
        elif process["pid"] and process["guid"]:
            pgm.add_process(process)
