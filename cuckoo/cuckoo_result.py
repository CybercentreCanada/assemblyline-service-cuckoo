...
def generate_al_result(api_report: Dict[str, Any], al_result: ResultSection, file_ext: str,
    ...
    if network:
        process_network(network, al_result, validated_random_ip_range, process_map, events)
...
def process_signatures(sigs: List[Dict[str, Any]], parent_result_section: ResultSection, inetsim_network: IPv4Network,
    ...
    for sig in sigs:
        ...
        if _is_signature_a_false_positive(sig_name, sig_marks, target_filename, target_filename_remainder, inetsim_network):
            continue
...
def _is_signature_a_false_positive(name: str, marks: List[Dict[str, Any]], filename: str, filename_remainder: str,
                                   inetsim_network: IPv4Network) -> bool:
    """
    ...
    for mark in marks:
        if name == "creates_doc" and (filename in mark.get("ioc") or filename_remainder in mark.get("ioc")):
            ...
            if contains_safelisted_value(http_string[1]):
                fp_count += 1
        elif name == "nolookup_communication" and mark["type"] == "generic":
            if contains_safelisted_value(mark["host"]) or (is_ip(mark["host"]) and ip_address(mark["host"]) in inetsim_network):
                fp_count += 1
            ...
            for item in mark:
                if item not in SKIPPED_MARK_ITEMS and \
                        (contains_safelisted_value(mark[item]) or
            ...
            if category and category not in SKIPPED_CATEGORY_IOCS:
                if contains_safelisted_value(ioc):
                    fp_count += 1
